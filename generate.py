...
def get_anonymous_token(region: str = "us") -> tuple[str | None, str | None]:
    ...
    Uses PLEX_TOKEN environment variable (GitHub Secret) if available,
    otherwise falls back to anonymous token endpoint.
    """
    client_id = _new_client_id()
    
    # Try stored token first (GitHub Secret)
    stored_token = os.environ.get("PLEX_TOKEN")
    if stored_token:
        logger.info("Using stored PLEX_TOKEN for region=%s", region)
        return stored_token, client_id

    # Fallback: anonymous token endpoint
